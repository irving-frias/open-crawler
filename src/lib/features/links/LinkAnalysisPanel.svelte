<script lang="ts">
  ...
  import type { LinkAnalysis, SeoCheckResult, SeoOverview } from '$lib/api/types';
      ...
      <CardContent>
        ...
        <div class="links-grid">
          <div class="links-section">
            <h4 class="links-section-title">{m['links.orphans']()}</h4>
            <div class="links-section-body">
              <Badge variant={links.orphan_count === 0 ? 'default' : 'warning'}>
                {links.orphan_count.toLocaleString()}
              </Badge>
              {#if links.orphan_pages.length > 0}
                <ul class="links-list">
                  {#each links.orphan_pages.slice(0, 10) as url (url)}
          ...
          <div class="links-section">
            <h4 class="links-section-title">{m['links.dead_ends']()}</h4>
            <div class="links-section-body">
              <Badge variant={links.dead_end_count === 0 ? 'default' : 'warning'}>
                {links.dead_end_count.toLocaleString()}
              </Badge>
              {#if links.dead_end_pages.length > 0}
                <ul class="links-list">
                  {#each links.dead_end_pages.slice(0, 10) as url (url)}
                    <li class="links-list-item" title={url}>{url}</li>
                  {/each}
                </ul>
              {/if}
        ...
        <div class="links-grid">
          ...
          {#if links.top_anchors.length > 0}
            <div class="links-section">
              <h4 class="links-section-title">{m['links.top_anchors']()}</h4>
              <div class="links-section-body">
                <ul class="links-list">
                  {#each links.top_anchors.slice(0, 10) as a (a.anchor)}
        ...
        {#if links.external_domains.length > 0}
          <div class="links-section">
            <h4 class="links-section-title">
              {m['links.external_domains']()}
              <span class="links-section-count">
                {m['links.sample']({
                  shown: Math.min(links.external_domains.length, 10),
                  total: links.external_domains.length,
                })}
              </span>
            </h4>
            <div class="links-section-body">
              <ul class="links-list">
                {#each links.external_domains.slice(0, 10) as d (d.domain)}
                  <li class="links-list-row">
