...
## CURRENT TASK: Data Sharing between devices (WiFi + Bluetooth) — IN PROGRESS

**Goal:** Export & share the app's data (projects + results + configs) between devices via an **importable `.ocproj` package**, transferable over **WiFi (LAN HTTP server + QR)** and **Bluetooth/Nearby (Android share sheet + receive-as-share-target)**. All device pairs (Android↔Android, Desktop↔Android, Desktop↔Desktop). Receiver MUST be able to import. **Direct share**: choosing WiFi/Bluetooth exports the package automatically (no separate export step); Export/Import tabs stay as standalone features. **WebRTC P2P over internet was REMOVED (S8)** — LAN WiFi and Bluetooth/Nearby are the only cross-network channels.

**User decisions locked:**

- Data = app-data package (importable), NOT just reports.
- Channels = WiFi LAN+QR AND Bluetooth/Nearby (P2P over internet removed in S8).
...
- P2P plan was approved (WebRTC via PeerJS, `0.peerjs.com`, TURN fallback) and implemented in Sprint 7, then **REMOVED in S8** — the feature is no longer used. No libp2p fallback. Linux is covered by the LAN WiFi receiver page.
...
- `p2p.rs` (P5, plan approved, NOT started, now **CANCELLED with S8 removal**): libp2p = `0.56` (features `tcp, noise, yamux, relay, dcutr, mdns, kad, identify, tokio`), `Relay(client)` + `Identify` + `Dcutr` + `Mdns` + `Kademlia` + `StreamProtocol` streaming; rendezvous by relay multiaddr in QR or DHT `hash(code)`; DCUtR hole-punch after relay. Feature-flag in `Cargo.toml` (proposed default-on).

**Deep-link (QR/links):** WiFi links are plain URLs (`http://<ip>:45231/<token>`); no WebRTC codes. WiFi LAN receiver page handled in-app. Receiver writes to `<appData>/transfers/<name>` then calls `import_package(dest, mode)` (receiver picks conflict mode). Progress via `transfer-progress` event.
...
**P2P deps (S8, REMOVED):** JS `peerjs@1.5.5`, `jsQR`, `qrcode`, `@types/qrcode` removed from `package.json` (verified unused — `bun run build` passes without them). Rust `libp2p = "0.56"` never added. Camera perms (`android.permission.CAMERA`, `NSCameraUsageDescription`) removed.

**Optimización memory leaks (A+B):** persistencia de config en `app.svelte.ts` guarda credenciales **sanitizadas** (proxyPass/sitePass/cookies excluidas; proxyUser/siteUser conservadas). Cache de resultados **sin `html_body`** (usa `row_to_result_light`, nunca selecciona el HTML). `CrawlRepo<'a>` **presta** la `ResultsCacheArc` (antes `clone()` por llamada a `with_repo`). Re-audit SEO concurrente con `buffer_unordered(4)` (`SEO_AUDIT_CONCURRENCY`, `Arc<HttpFetcher>`, escrituras DB serializadas). `db_writer` fallback por página/link si el INSERT batch falla (transacción individual + `total_errors`). **Export XLSX/CSV fuera del lock de DB** — `export_xlsx`/`export_csv_single` son async y leen cada lote con `with_repo` corto (keyset `get_result_batch`/`get_links_batch`), escribiendo el workbook/CSV fuera del lock. Transfer server: `Drop` para `TransferServerState`, `start_transfer_server` detiene el anterior + acepta `transfers_dir`, limpia el archivo managed al parar, `sweep_stale_transfers` (`.ocproj` > 24h). Snapshots podados a **12 por proyecto** (`MAX_SNAPSHOTS_PER_PROJECT`) en la misma transacción. SEO overview por SQL (migración 011, `seo_normalized` + backfill). Resultado: **104 tests, 0 clippy warnings.**
...
**F4 WebRTC P2P over internet — `transfer/p2p.ts`** — **DONE in S7, REMOVED in S8** (feature no longer used). PeerJS sender/receiver, jsQR camera scanning, `ocp2p:` codes, LAN `/receive` browser page, P2P state/actions in `app.svelte.ts`, "Internet (P2P)" TransferDialog tab, `0.peerjs.com` CSP, `CAMERA` + `NSCameraUsageDescription` perms — **all deleted**.
...
- **P4 WebRTC P2P (PeerJS)** — DONE in S7, then **REMOVED in S8** (feature no longer used): `transfer/p2p.ts`, `/receive` route + helpers in `server.rs`, P2P state/actions in `app.svelte.ts`, `transfer.p2p.*` i18n keys, `peerjs`/`jsqr`/`qrcode`/`@types/qrcode` deps, camera perms + `0.peerjs.com` CSP all removed. WiFi LAN + Bluetooth/Nearby unchanged.
- **P5 libp2p (Linux/native)** — **CANCELLED with S8 removal** (Linux P2P covered by LAN WiFi receiver page).
...
_Last updated: Sprint 7 complete — release profile (strip/lto=fat/cgu=1/panic=abort) + native-tls removed (rustls-only), Android network security config for cleartext LAN/localhost, WebRTC P2P over internet (PeerJS sender+receiver, QR scan, LAN `/receive` browser page) wired into TransferDialog, real OBEX Object Push over RFCOMM for Linux+Windows (`transfer/obex/` codec + transports, `bt_send` command, Bluetooth tab UI) — OBEX untested on hardware. **Sprint 8: WebRTC P2P over internet REMOVED entirely** (`transfer/p2p.ts`, `/receive` + helpers in `server.rs`, P2P state/actions in `app.svelte.ts`, `transfer.p2p.*` i18n keys, `peerjs`/`jsqr`/`qrcode`/`@types/qrcode` deps, camera perms + `0.peerjs.com` CSP). WiFi LAN + Bluetooth/Nearby sharing unchanged. Next: verify OBEX on a real device, README transfer section. **Sprint 9 (optimización A+B):** cache sin html_body + `CrawlRepo` presta la cache, re-audit SEO `buffer_unordered(4)`, `db_writer` fallback por página, export XLSX/CSV fuera del lock de DB, transfer server con semáforo + sweep 24h, snapshots podados a 12, SEO overview por SQL — 104 tests, clippy clean. Next: SEO Fase 4 (migraciones 009/010 + repos + tab "SEO Insights")._
